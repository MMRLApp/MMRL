...
constructor(
    ...
    private fun loadData() =
        viewModelScope.launch {
            ...
            if (installed && local?.updateJson != null) {
                // TODO: get rid of !!
        ...
        State.ENABLE ->
            ModuleOps(
                ...
                toggle = {
                    ...
                    module.disable()
                        .onSuccess { opsTasks.remove(it.id) }
                        .onFailure {
                            Log.e(TAG, "Failed to disable module: ${it.message}")
                ...
                change = {
                    ...
                    module.remove()
                        .onSuccess { opsTasks.remove(it.id) }
                        .onFailure {
                            Log.e(TAG, "Failed to remove module: ${it.message}")
                            opsTasks.remove(module.id)
                        }
            ...
            ModuleOps(
                ...
                toggle = {
                    ...
                    module.enable()
                        .onSuccess { opsTasks.remove(it.id) }
                        .onFailure {
                            Log.e(TAG, "Failed to enable module: ${it.message}")
                            opsTasks.remove(module.id)
                        }
                },
                change = {
