...
fun String.findFileGlob(
    prefix: String,
    vararg patterns: String = arrayOf("*.apk", "*.jar", "*.dex"),
): Path? {
...
inline fun <R> String?.ifNotNullOrBlank(block: (String) -> R): R? = if (!this.isNullOrBlank()) block(this) else null
...
fun String.toDecodedUrl(force: Boolean = false): String =
    if (force || isUrlEncoded(this)) {
